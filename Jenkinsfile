        ...
        stage('DeployToProduction') {
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'servers-login', usernameVariable: 'USERNAME', passwordVariable: 'USERPASS')]) {
                    script {
                        sh "sshpass -p '$USERPASS' -v ssh -o StrictHostKeyChecking=no $USERNAME@$prod_ip \"docker pull rodozur/train-schedule:${env.BUILD_NUMBER}\""
                        try {
                        ...
                        sh "sshpass -p '$USERPASS' -v ssh -o StrictHostKeyChecking=no $USERNAME@$prod_ip \"docker run --restart always --name train-schedule -p 8080:8080 -d rodozur/train-schedule:${env.BUILD_NUMBER}\""
                    }
                }
            }
        }
